...
const RecommendationCard = ({ rec, onAction }) => {
  const getIcon = (category) => {
    switch (category) {
      case 'Savings':
        return (
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-fin-success/10 text-fin-success border border-fin-success/20">
      ...
      case 'Tax Optimizer':
        return (
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-fin-primary/12 text-fin-primary border border-fin-primary/25">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="2.5" stroke="currentColor" className="h-5 w-5">
      ...
      case 'Food':
        return (
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-fin-warning/12 text-fin-warning border border-fin-warning/25">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="2.5" stroke="currentColor" className="h-5 w-5">
  ...
  const getImpactBadge = (impact) => {
    switch (impact) {
      case 'Critical':
        return <span className="rounded-full bg-fin-danger/12 border border-fin-danger/25 px-2.5 py-0.5 text-[9px] font-extrabold text-fin-danger uppercase tracking-wider">Critical Impact</span>
      case 'High':
        return <span className="rounded-full bg-fin-warning/12 border border-fin-warning/25 px-2.5 py-0.5 text-[9px] font-extrabold text-fin-warning uppercase tracking-wider">High Impact</span>
      default:
        return <span className="rounded-full bg-fin-primary/12 border border-fin-primary/25 px-2.5 py-0.5 text-[9px] font-extrabold text-fin-primary uppercase tracking-wider">Medium Impact</span>
    }
  }
    ...
    <Card className="flex flex-col justify-between h-full space-y-4">
      ...
      <div className="pt-3.5 border-t border-fin-border">
        <button
          ...
          className="text-xs font-black text-fin-success hover:text-fin-success/90 transition flex items-center gap-1.5 cursor-pointer uppercase tracking-wider"
        >
          {rec.action}
...
const HealthScoreCard = ({ score, status, suggestions, totalIncome, totalExpenses }) => {
  ...
  const getScoreColor = (val) => {
    if (val >= 85) return 'text-fin-success'
    if (val >= 70) return 'text-fin-success'
    if (val >= 50) return 'text-fin-warning'
    return 'text-fin-danger'
  }

  const getScoreBgColor = (val) => {
    if (val >= 85) return 'stroke-fin-success/10'
    if (val >= 70) return 'stroke-fin-success/5'
    if (val >= 50) return 'stroke-fin-warning/10'
    return 'stroke-fin-danger/10'
  }

  const getScoreStroke = (val) => {
    if (val >= 85) return 'stroke-fin-success'
    if (val >= 70) return 'stroke-fin-success'
    if (val >= 50) return 'stroke-fin-warning'
    return 'stroke-fin-danger'
  }
      ...
      <div className="flex flex-col items-center justify-center p-4 border-fin-border md:border-r space-y-3">
        ...
        <div>
          <span className={`inline-flex items-center rounded-full px-3 py-0.5 text-[10px] font-extrabold uppercase tracking-wider ${
            score >= 85 ? 'bg-fin-success/12 text-fin-success' :
            score >= 70 ? 'bg-fin-success/10 text-fin-success' :
            score >= 50 ? 'bg-fin-warning/12 text-fin-warning' : 'bg-fin-danger/12 text-fin-danger'
          }`}>
            {status}
          </span>
        </div>
      ...
      <div className="flex flex-col justify-center p-4 border-fin-border md:border-r space-y-4">
        ...
        <div className="space-y-3">
          <div className="flex justify-between items-center text-xs">
            ...
            <span className="font-bold text-fin-success">₹{totalIncome.toLocaleString('en-IN')}</span>
          </div>
          <div className="flex justify-between items-center text-xs">
            <span className="text-fin-text-muted font-medium">Outflow Rate:</span>
            <span className="font-bold text-fin-danger">₹{totalExpenses.toLocaleString('en-IN')}</span>
          </div>
          <div className="flex justify-between items-center text-xs border-t border-fin-border pt-3">
            ...
            <span className={`font-black ${totalIncome - totalExpenses >= 0 ? 'text-fin-success' : 'text-fin-danger'}`}>
...
const AIInsights = () => {
      ...
      <div className="min-h-screen bg-fin-bg text-fin-text-primary flex flex-col md:flex-row relative">
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <main className="flex-1 flex flex-col items-center justify-center space-y-4">
            <svg className="animate-spin h-9 w-9 text-fin-primary" fill="none" viewBox="0 0 24 24">
  ...
  const weekendAvg = patternData?.weekendAvg || 0
  const weekdayAvg = patternData?.weekdayAvg || 0
      ...
      <div className="flex-1 flex flex-col min-w-0 z-10">
        ...
        >
          ...
          <div className="grid gap-6 md:grid-cols-2">
              ...
              {patternData && (
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="h-2 w-full rounded-full bg-fin-border overflow-hidden">
                      <motion.div
                        initial={{ width: 0 }}
                        animate={{ width: `${Math.min(100, Math.max(10, savingsRatio))}%` }}
                        transition={{ duration: 0.8 }}
                        className="h-full rounded-full bg-fin-success"
                      />
                    </div>
                  </div>
                  ...
                  <div className={`p-4 rounded-xl border flex items-start gap-3.5 transition duration-300 ${
                    patternData.weekendOverspending
                      ? 'border-fin-danger/20 bg-fin-danger/8 text-fin-text-primary'
                      : 'border-fin-border bg-fin-hover-bg dark:bg-fin-card text-fin-text-primary'
                  }`}>
                    <div className={`mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-sm font-bold shadow-fin-sm ${
                      patternData.weekendOverspending ? 'bg-fin-danger/12 text-fin-danger' : 'bg-fin-input-bg dark:bg-fin-input-bg text-fin-text-muted'
                    }`}>
                    ...
                    <div className="text-xs space-y-1">
                      ...
                      <p className="text-fin-text-muted leading-relaxed">
                        {patternData.weekendOverspending
                            ? `Your weekend transactions average ₹${weekendAvg.toLocaleString('en-IN')}, which is significantly higher than your weekday average of ₹${weekdayAvg.toLocaleString('en-IN')}. Consider capping weekend outflows.`
                            : `Excellent consistency! Your average weekend expense (₹${weekendAvg.toLocaleString('en-IN')}) remains balanced compared to weekdays (₹${weekdayAvg.toLocaleString('en-IN')}).`}
                      </p>
                    </div>
                  </div>
                    ...
                    {patternData.spendingSpikes?.length > 0 ? (
                      <div className="space-y-2">
                        {patternData.spendingSpikes.map((spike, idx) => (
                          <div key={`spike-${idx}`} className="flex justify-between items-center rounded-xl border border-fin-border p-3.5 bg-fin-hover-bg dark:bg-fin-card text-xs shadow-2xs">
                            ...
                            <div className="text-right">
                              <p className="font-black text-fin-danger">₹{spike.amount.toLocaleString('en-IN')}</p>
                              <p className="text-[9px] font-bold text-fin-danger bg-fin-danger/12 px-2 py-0.5 rounded-full mt-1.5 inline-block">
            ...
            <Card hoverable={false} className="space-y-4">
              <h3 className="text-sm font-bold text-fin-text-primary flex items-center gap-2 border-b border-fin-border pb-3">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="2.5" stroke="currentColor" className="h-5 w-5 text-fin-success">
              ...
              {patternData && (
                <div className="space-y-3">
                  ...
                  {patternData.recurringExpenses?.length > 0 ? (
                    <div className="space-y-2">
                      {patternData.recurringExpenses.map((sub, idx) => (
                        <div key={`rec-${idx}`} className="flex justify-between items-center rounded-xl border border-fin-border p-3.5 bg-fin-hover-bg dark:bg-fin-card text-xs shadow-2xs">
                          <div className="flex items-center space-x-2.5">
                            <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-fin-success/15 text-fin-success font-black">
                              ⚡
                            </div>
                            <div className="space-y-0.5">
                  ...
                  {/* smart tip */}
                  <div className="rounded-xl bg-fin-success/8 border border-fin-success/20 p-3.5 flex gap-3 text-xs text-fin-text-primary">
                    <span className="text-base shrink-0">💡</span>
                    <div className="space-y-1">
                      <p className="font-extrabold text-fin-success">Proactive Optimization Tip</p>
                      <p className="text-fin-text-muted leading-relaxed">
